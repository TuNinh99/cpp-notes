...
1. Single (Đơn nhiệm): mỗi class chỉ nên đảm nhiệm 1 nhiệm vụ/chức năng
...
2. Open/Closed (mở/đóng): mở rộng để phát triển, đóng để sửa đổi
...
3. Liskov (Thay thế Liskov): các đối tượng của lớp con phải có khả năng thay thế lớp cha mà không làm thay đổi tính đúng đắn của chương trình
...
4. Interface (Phân tách interface): thay vì dùng interface lớn hãy dùng nhiều interface nhỏ chuyên biệt
...
5. Dependency (Đảo ngược phụ thuộc): các module cấp cao không nên phụ thuộc vào các module cấp thấp, cả 2 nên phụ thuộc vào abstraction (interface)
